...
private:
    ...
    // MEMBERS
#ifdef VL_RT_THREAD
    rt_mutex_t m_mutex = nullptr;
    rt_sem_t m_sem = nullptr;
    rt_sem_t m_exit_sem = nullptr;
    rt_thread_t m_thread = nullptr;
#else
    mutable VerilatedMutex m_mutex;
    std::condition_variable_any m_cv;
#endif
    // Only notify waiter if the worker is waiting
...
#endif
#ifdef VL_RT_THREAD
    VerilatedContext* m_contextp = nullptr;
#endif
...
public:
...
#ifdef VL_RT_THREAD
        while (true) {
            if (rt_mutex_take(m_mutex, RT_WAITING_FOREVER) != RT_EOK) continue;
            if (!m_ready.empty()) {
                m_waiting = false;
                *workp = m_ready.front();
            ...
            m_waiting = true;
            rt_mutex_release(m_mutex);
            rt_sem_take(m_sem, RT_WAITING_FOREVER);
        }
#else
    ...
    void addTask(VlExecFnp fnp, VlSelfP selfp, bool evenCycle = false)
        ...
        {
#ifdef VL_RT_THREAD
            rt_mutex_take(m_mutex, RT_WAITING_FOREVER);
    ...
    void workerLoop();
#ifdef VL_RT_THREAD
    static void rtThreadTrampoline(void* parameter);
#else
