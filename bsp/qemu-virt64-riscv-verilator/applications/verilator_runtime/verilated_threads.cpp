...
void VlMTaskVertex::yieldThread() {
    ++s_yields;  // Statistics
#ifdef VL_RT_THREAD
    rt_thread_yield();
#else
...
{
#ifdef VL_RT_THREAD
    m_mutex = rt_mutex_create("vlmtx", RT_IPC_FLAG_FIFO);
    m_sem = rt_sem_create("vlsem", 0, RT_IPC_FLAG_FIFO);
    m_exit_sem = rt_sem_create("vlexit", 0, RT_IPC_FLAG_FIFO);
    m_thread = rt_thread_create("vlw",
                                &VlWorkerThread::rtThreadTrampoline,
                                this,
                                4096,
                                20,
                                10);
    if (m_thread) rt_thread_startup(m_thread);
#endif
}
...
#ifdef VL_RT_THREAD
    if (m_exit_sem) {
        rt_sem_take(m_exit_sem, RT_WAITING_FOREVER);
    }
    if (m_thread) {
...
void VlWorkerThread::wait() {
    ...
    // Yield wait
    while (!flag.load()) VlMTaskVertex::yieldThread();
}
...
#endif
#ifdef VL_RT_THREAD
void VlWorkerThread::rtThreadTrampoline(void* parameter) {
    auto* workerp = static_cast<VlWorkerThread*>(parameter);
    if (!workerp) return;
    Verilated::threadContextp(workerp->m_contextp);
    workerp->workerLoop();
    if (workerp->m_exit_sem) rt_sem_release(workerp->m_exit_sem);
}
#endif
...
#ifdef VL_RT_THREAD
    return "rt-thread: affinity not supported";
#elif defined(__linux) || defined(CPU_ZERO) || defined(VL_CPPCHECK)  // Linux-like pthreads
