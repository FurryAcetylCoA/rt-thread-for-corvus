...
#ifdef VL_RT_THREAD
static inline char* vlrt_strdup(const char* namep) {
    if (!namep) return nullptr;
    const size_t len = std::strlen(namep) + 1;
    char* const bufp = static_cast<char*>(std::malloc(len));
    if (bufp) std::memcpy(bufp, namep, len);
    return bufp;
}

static inline double vlrt_exp2(double v) { return std::pow(2.0, v); }

static uint32_t vlrt_rand_state = 1u;
static inline uint32_t vlrt_rand32() {
    vlrt_rand_state = 1664525u * vlrt_rand_state + 1013904223u;
    return vlrt_rand_state;
}
#else
static inline double vlrt_exp2(double v) { return std::exp2(v); }
#endif
...
// Internal note: Globals may multi-construct, see verilated.cpp top.
Verilated::ThreadLocal Verilated::t_s;
...
uint32_t VL_THREAD_ID() VL_MT_SAFE {
    ...
    static uint32_t t_myId = ++s_nextId;
    return t_myId;
}
...
VlRNG& VlRNG::vl_thread_rng() VL_MT_SAFE {
    static VlRNG t_rng{0};
    static uint32_t t_seedEpoch = 0;
...
double VL_ITOR_D_W(int lbits, const WDataInP lwp) VL_PURE {
    ...
    const double hi = static_cast<double>(ihi) * vlrt_exp2(2 * VL_EDATASIZE);
    const double mid = static_cast<double>(imid) * vlrt_exp2(VL_EDATASIZE);
    const double lo = static_cast<double>(ilo);
    const double d = (hi + mid + lo) * vlrt_exp2(VL_EDATASIZE * (ms_word - 2));
    return d;
}
...
void _vl_vsformat(std::string& output, const std::string& format, va_list ap) VL_MT_SAFE {
    ...
    static char t_tmp[VL_VALUE_STRING_MAX_WIDTH];
...
void VL_SFORMAT_NX(int obits, CData& destr, const std::string& format, int argc, ...) VL_MT_SAFE {
    static std::string t_output;  // static only for speed
    t_output = "";
    va_list ap;
    va_start(ap, argc);
...
const char* vl_mc_scan_plusargs(const char* prefixp) VL_MT_SAFE {
    ...
    static char t_outstr[VL_VALUE_STRING_MAX_WIDTH];
...
const char* Verilated::catName(const char* n1, const char* n2, const char* delimiter) VL_MT_SAFE {
    ...
    static char* t_strp = nullptr;
    static size_t t_len = 0;
