...
{
    rt_kprintf("[TB] entering main\n");
    int rc = EXIT_FAILURE;
    try
    {
        Verilated::commandArgs(argc, argv);
        rt_kprintf("[TB] commandArgs done argc=%d argv=%p\n", argc, argv);

        auto context = std::make_unique<VerilatedContext>();
        context->threads(1);  // Force single-threaded to avoid std::thread usage on RT-Thread
        context->traceEverOn(false);
        rt_kprintf("[TB] VerilatedContext created ctx=%p\n", context.get());
        
        auto dut = std::make_unique<Vdut_162>(context.get());
        rt_kprintf("[TB] DUT constructed dut=%p\n", dut.get());
    ...
    auto apply_reset = [&]() {
        ...
        rt_kprintf("[TB] applying reset\n");
        apply_reset();
        rt_kprintf("[TB] reset done\n");

    auto run_cycle = [&](const Inputs &in, const std::string &label) -> bool {
        std::cout << "[TB] step " << step_counter++ << " begin: " << label << std::endl;
        rt_kprintf("[TB] step %d begin: %s\n", step_counter, label.c_str());
            ...
            dut->predict_history != expected_history)
        {
            std::cerr << "[TB] dut_162 predict mismatch (" << label << "): "
                      ...
                      << std::endl;
            rt_kprintf("[TB] predict mismatch at step %d\n", step_counter);
            return false;
        }
    ...
    {
        rt_kprintf("[TB] exception: %s\n", ex.what());
    }
    catch (...)
    {
        rt_kprintf("[TB] unknown exception\n");
    }

exit_main:
    rt_kprintf("[TB] main returning %d\n", rc);
    // Stay alive so we can observe logs without hitting C++ exit/cleanup paths
    while (true)
    {
